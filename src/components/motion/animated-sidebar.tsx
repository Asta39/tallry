"use client";
// Adapted from beui.dev/components/motion/animated-sidebar — link items use
// next/link (client-side navigation) instead of a plain <a>.
...
>(function AnimatedSidebarMenu(
  ...
  return (
    <SharedLayoutBg
      ...
      as="ul"
      inset={0}
      pillClassName="rounded-xl bg-white/50"
...
export function AnimatedSidebarMenuButton({
  ...
  const select = (
    ...
    // leaves its children unreachable — selecting a group unfolds the panel
    // that is about to hold it.
    ...
    <>
      {isActive ? (
        <motion.span
          ...
          className="absolute inset-0 rounded-xl bg-white/80 shadow-[0_1px_2px_rgba(0,0,0,0.05)]"
        />
      ) : null}
      {icon ? (
        <span
